  ...
  return (
    <div>
      <h2 id="comment-heading" data-cy="CommentHeading">
        <Translate contentKey="booksApp.comment.home.title">Comments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="booksApp.comment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/comment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="booksApp.comment.home.createLabel">Create new Comment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {commentList && commentList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="booksApp.comment.id">ID</Translate>
                </th>
                <th>
                  <Translate contentKey="booksApp.comment.text">Text</Translate>
                </th>
                <th>
                  <Translate contentKey="booksApp.comment.book">Book</Translate>
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {commentList.map((comment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{comment.text}</td>
                  <td>{comment.book ? <Link to={`/book/${comment.book.id}`}>{comment.book.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="booksApp.comment.home.notFound">No Comments found</Translate>
            </div>
          )
        )}
      </div>
